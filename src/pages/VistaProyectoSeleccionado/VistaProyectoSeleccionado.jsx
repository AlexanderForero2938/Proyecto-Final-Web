...
const VistaProyectoSeleccionado = () => {
  ...
  useEffect(() => {
    const fetchProyecto = async () => {
      ...
      const { data: proyectoData, error: proyectoError } =
        await supabase.rpc('informacion_proyecto', { prid: proyectoId });
      if (proyectoError) {
      ...
      const { data: estudiantesData, error: estudiantesError } =
        await supabase.rpc('informacion_estudiante_proyecto', { prid: proyectoId });
      if (estudiantesError) {
      ...
      const { data: objetivosData, error: objetivosError } =
        await supabase.rpc('informacion_objetivos', { prid: proyectoId });
      if (objetivosError) {
      ...
      const { data: estadoData, error: estadoError } =
        await supabase.rpc('estadoproyecto', { pid: proyectoId });
      if (estadoError) {
      ...
      const { data: historialData, error: historialError } =
        await supabase.rpc('historial_estado', { pid: proyectoId });
      if (historialError) {
  ...
  return (
    <div id='contenedor-principal-proyecto-docente'>
      <MenuDocente />
      <div id='contenedor-titulo-proyecto'>
        ...
        <button
          className='btn-pdf'
          onClick={() =>
            generateProjectPDF({ proyecto, estudiantes, objetivos, estado, historialEstados })
          }
        >
          Descargar PDF
        </button>
      ...
      <div className='contenedor-informacion'>
        ...
        <ul>
          {estudiantes.length === 0 ? (
            ...
            estudiantes.map((est, index) => <li key={index}>{est.nombre}</li>)
          )}
        </ul>
      </div>
